...
const Puzzle8 = () => {
  ...
  const [time, setTime] = useState(
    Number(localStorage.getItem("timeLeftPuzzle8")) || 300
  ); // 5 minutes in seconds
  const [answer, setAnswer] = useState("");
  const questions = JSON.parse(localStorage.getItem("questions"));
  const correctAnswers = questions[7].answers;
  const question = questions[7].text;
  const hints = questions[7].hints;

  useEffect(() => {
    const currentUrl = localStorage.getItem("currentURL");
    if (currentUrl !== "/Puzzle8") {
      navigate(currentUrl);
    }
    let intervalId;
    if (time > 0) {
      ...
      localStorage.setItem("currentURL", "/Puzzle9");
      navigate("/Puzzle9");
    }
    return () => {
      clearInterval(intervalId);
      localStorage.setItem("timeLeftPuzzle8", time.toString());
    };
  }, [time, navigate]);
  ...
  const handleSubmit = (event) => {
    ...
    if (correctAnswers.includes(answer.toLowerCase())) {
      ...
      localStorage.setItem("point8", puzzleScore);
      setAnswer("");
    }
    localStorage.setItem("currentURL", "/Puzzle9");
    navigate("/Puzzle9");
  ...
  return (
    <div className="puzzle-container">
      <div className="timer">
        <h2 className="question">Puzzle 8</h2>
        <div>
          Timer: &nbsp;
          <span id="timer">
            {Math.floor(time / 60)}:
