...
const Puzzle12 = () => {
  ...
  const [time, setTime] = useState(
    Number(localStorage.getItem("timeLeftPuzzle12")) || 300
  ); // 5 minutes in seconds
  const [answer, setAnswer] = useState("");
  const questions = JSON.parse(localStorage.getItem("questions"));
  const correctAnswers = questions[11].answers;
  const question = questions[11].text;
  const hints = questions[11].hints;

  useEffect(() => {
    const currentUrl = localStorage.getItem("currentURL");
    if (currentUrl !== "/Puzzle12") {
      navigate(currentUrl);
    }
    let intervalId;
    if (time > 0) {
    ...
    return () => {
      clearInterval(intervalId);
      localStorage.setItem("timeLeftPuzzle12", time.toString());
    };
  }, [time, navigate]);
  ...
  const handleSubmit = (event) => {
    ...
    if (correctAnswers.includes(answer.toLowerCase())) {
      ...
      localStorage.setItem("point12", puzzleScore);
      setAnswer("");
    }
    localStorage.setItem("currentURL", "/result");
    navigate("/result");
  ...
  return (
    <div className="puzzle-container">
      <div className="timer">
        <h2 className="question">Puzzle 12</h2>
        <div>
          Timer: &nbsp;
          <span id="timer">
            {Math.floor(time / 60)}:
