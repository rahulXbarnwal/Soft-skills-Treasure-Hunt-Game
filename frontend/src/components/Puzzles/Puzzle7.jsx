...
const Puzzle7 = () => {
  ...
  const [time, setTime] = useState(
    Number(localStorage.getItem("timeLeftPuzzle7")) || 300
  ); // 5 minutes in seconds
  const [answer, setAnswer] = useState("");
  const questions = JSON.parse(localStorage.getItem("questions"));
  const correctAnswers = questions[6].answers;
  const question = questions[6].text;
  const hints = questions[6].hints;

  useEffect(() => {
    const currentUrl = localStorage.getItem("currentURL");
    if (currentUrl !== "/Puzzle7") {
      navigate(currentUrl);
    }
    let intervalId;
    if (time > 0) {
      ...
      localStorage.setItem("currentURL", "/Puzzle8");
      navigate("/Puzzle8");
    }
    return () => {
      clearInterval(intervalId);
      localStorage.setItem("timeLeftPuzzle7", time.toString());
    };
  }, [time, navigate]);
  ...
  const handleSubmit = (event) => {
    ...
    if (correctAnswers.includes(answer.toLowerCase())) {
      ...
      localStorage.setItem("point7", puzzleScore);
      setAnswer("");
    }
    localStorage.setItem("currentURL", "/Puzzle8");
    navigate("/Puzzle8");
  ...
  return (
    <div className="puzzle-container">
      <div className="timer">
        <h2 className="question">Puzzle 7</h2>
        <div>
          Timer: &nbsp;
          <span id="timer">
            {Math.floor(time / 60)}:
