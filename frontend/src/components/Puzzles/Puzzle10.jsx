...
const Puzzle10 = () => {
  ...
  const [time, setTime] = useState(
    Number(localStorage.getItem("timeLeftPuzzle10")) || 300
  ); // 5 minutes in seconds
  const [answer, setAnswer] = useState("");
  const questions = JSON.parse(localStorage.getItem("questions"));
  const correctAnswers = questions[9].answers;
  const question = questions[9].text;
  const hints = questions[9].hints;

  useEffect(() => {
    const currentUrl = localStorage.getItem("currentURL");
    if (currentUrl !== "/Puzzle10") {
      navigate(currentUrl);
    }
    let intervalId;
    if (time > 0) {
      ...
      localStorage.setItem("currentURL", "/Puzzle11");
      navigate("/Puzzle11");
    }
    return () => {
      clearInterval(intervalId);
      localStorage.setItem("timeLeftPuzzle10", time.toString());
    };
  }, [time, navigate]);
  ...
  const handleSubmit = (event) => {
    ...
    if (correctAnswers.includes(answer.toLowerCase())) {
      ...
      localStorage.setItem("point10", puzzleScore);
      setAnswer("");
    }
    localStorage.setItem("currentURL", "/Puzzle11");
    navigate("/Puzzle11");
  ...
  return (
    <div className="puzzle-container">
      <div className="timer">
        <h2 className="question">Puzzle 10</h2>
        <div>
          Timer: &nbsp;
          <span id="timer">
            {Math.floor(time / 60)}:
