...
const Puzzle6 = () => {
  ...
  const [time, setTime] = useState(
    Number(localStorage.getItem("timeLeftPuzzle6")) || 300
  ); // 5 minutes in seconds
  const [answer, setAnswer] = useState("");
  const questions = JSON.parse(localStorage.getItem("questions"));
  const correctAnswers = questions[5].answers;
  const question = questions[5].text;
  const hints = questions[5].hints;

  useEffect(() => {
    const currentUrl = localStorage.getItem("currentURL");
    if (currentUrl !== "/Puzzle6") {
      navigate(currentUrl);
    }
    let intervalId;
    if (time > 0) {
      ...
      localStorage.setItem("currentURL", "/Puzzle7");
      navigate("/Puzzle7");
    }
    return () => {
      clearInterval(intervalId);
      localStorage.setItem("timeLeftPuzzle6", time.toString());
    };
  }, [time, navigate]);
  ...
  const handleSubmit = (event) => {
    ...
    if (correctAnswers.includes(answer.toLowerCase())) {
      ...
      localStorage.setItem("point6", puzzleScore);
      setAnswer("");
    }
    localStorage.setItem("currentURL", "/Puzzle7");
    navigate("/Puzzle7");
  ...
  return (
    <div className="puzzle-container">
      <div className="timer">
        <h2 className="question">Puzzle 6</h2>
        <div>
          Timer: &nbsp;
          <span id="timer">
            {Math.floor(time / 60)}:
